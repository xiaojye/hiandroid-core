 ...
 * @since 1.0
 */
public final class HiDisplayUtils {
...
//    private static int[] sLandscapeRealSizeCache = null;
    ...
    public static int getScreenHeight(Context context) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            if (HiDeviceUtils.isXiaomi() && xiaomiNavigationGestureEnabled(context)) {
...
//        }
    ...
    public static int getUsefulScreenWidth(Activity activity) {
        return getUsefulScreenWidth(activity, HiNotchHelper.hasNotch(activity));
    }

    public static int getUsefulScreenWidth(View view) {
        return getUsefulScreenWidth(view.getContext(), HiNotchHelper.hasNotch(view));
    }
    ...
    public static int getUsefulScreenWidth(Context context, boolean hasNotch) {
        ...
        if (!hasNotch) {
            if (isLandscape && HiDeviceUtils.isEssentialPhone()
                ...
                result -= 2 * HiStatusBarUtils.getStatusbarHeight(context);
            }
            return result;
        }
        if (isLandscape) {
            // 华为挖孔屏横屏时，会把整个 window 往后移动，因此，可用区域减小
            if (HiDeviceUtils.isHuawei() && !HiDisplayUtils.huaweiIsNotchSetToShowInSetting(context)) {
                result -= HiNotchHelper.getNotchSizeInHuawei(context)[1];
            }
    ...
    public static int getUsefulScreenHeight(Activity activity) {
        return getUsefulScreenHeight(activity, HiNotchHelper.hasNotch(activity));
    }

    public static int getUsefulScreenHeight(View view) {
        return getUsefulScreenHeight(view.getContext(), HiNotchHelper.hasNotch(view));
    }
    ...
    private static int getUsefulScreenHeight(Context context, boolean hasNotch) {
        ...
        if (!hasNotch) {
            if (isPortrait && HiDeviceUtils.isEssentialPhone()
                    && Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
                // https://arstechnica.com/gadgets/2017/09/essential-phone-review-impressive-for-a-new-company-but-not-competitive/
                // 这里说挖孔屏是状态栏高度的两倍
                result -= 2 * HiStatusBarUtils.getStatusbarHeight(context);
