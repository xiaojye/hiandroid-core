 ...
 * @since 1.0
 */
public class HiNotchHelper {

    private static final String TAG = HiNotchHelper.class.getSimpleName();
    ...
    public static boolean has3rdNotch(Context context){
        if (HiDeviceUtils.isHuawei()) {
            return hasNotchInHuawei(context);
        } else if (HiDeviceUtils.isVivo()) {
            return hasNotchInVivo(context);
        } else if (HiDeviceUtils.isOppo()) {
            return hasNotchInOppo(context);
        } else if (HiDeviceUtils.isXiaomi()) {
    ...
    private static Rect get3rdSafeInsetRect(Context context){
        // 全面屏设置项更改
        if (HiDeviceUtils.isHuawei()) {
            boolean isHuaweiNotchSetToShow = HiDisplayUtils.huaweiIsNotchSetToShowInSetting(context);
    ...
    private static Rect getRectInfoRotation0(Context context) {
        Rect rect = new Rect();
        if (HiDeviceUtils.isVivo()) {
            // TODO vivo 显示与亮度-第三方应用显示比例
            rect.top = getNotchHeightInVivo(context);
            rect.bottom = 0;
        } else if (HiDeviceUtils.isOppo()) {
            // TODO OPPO 设置-显示-应用全屏显示-凹形区域显示控制
            rect.top = HiStatusBarUtils.getStatusbarHeight(context);
            rect.bottom = 0;
        } else if (HiDeviceUtils.isHuawei()) {
    ...
    private static Rect getRectInfoRotation90(Context context) {
        ...
        if (HiDeviceUtils.isVivo()) {
            ...
            rect.left = HiStatusBarUtils.getStatusbarHeight(context);
            rect.right = 0;
        } else if (HiDeviceUtils.isHuawei()) {
    ...
    private static Rect getRectInfoRotation180(Context context) {
        ...
        if (HiDeviceUtils.isVivo()) {
            ...
            rect.top = 0;
            rect.bottom = HiStatusBarUtils.getStatusbarHeight(context);
        } else if (HiDeviceUtils.isHuawei()) {
            int[] notchSize = getNotchSizeInHuawei(context);
            rect.top = 0;
    ...
    private static Rect getRectInfoRotation270(Context context) {
        ...
        if (HiDeviceUtils.isVivo()) {
            ...
            rect.right = HiStatusBarUtils.getStatusbarHeight(context);
            rect.left = 0;
        } else if (HiDeviceUtils.isHuawei()) {
            if (sHuaweiIsNotchSetToShow) {
    ...
    public static int getNotchHeightInXiaomi(Context context) {
        ...
        return HiDisplayUtils.getStatusBarHeight(context);
    }

    public static int getNotchWidthInVivo(Context context){
        return HiDisplayUtils.dp2px(context, 100);
    }

    public static int getNotchHeightInVivo(Context context){
        return HiDisplayUtils.dp2px(context, 27);
    }
    ...
    public static boolean needFixLandscapeNotchAreaFitSystemWindow(View view){
       return (HiDeviceUtils.isXiaomi() || HiDeviceUtils.isVivo()) && HiNotchHelper.hasNotch(view);
    }
