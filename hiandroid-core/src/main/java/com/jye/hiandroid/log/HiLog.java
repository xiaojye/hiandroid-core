...
import com.jye.hiandroid.util.HiStackTraceUtil;
...
public class HiLog {
    ...
    public static void log(@HiLogType.TYPE int type, Object... contents) {
        log(type, HiLogManager.getInstance().getConfig().globalTag(), contents);
    }
    ...
    public static void log(@NonNull HiLogConfig config, @HiLogType.TYPE int type, @NonNull String tag, Object... contents) {
        ...
        if (config.includeThread()) {
            String threadInfo = HiLogConfig.getThreadFormatter().format(Thread.currentThread());
            sb.append(threadInfo).append("\n");
        }
        if (config.stackTraceDepth() > 0) {
            String stackTrace = HiLogConfig.getStackTraceFormatter().format(
